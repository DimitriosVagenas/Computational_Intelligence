...
public class Perceptron3Layers {
	...
	private static double[][] newInputs4;
	...
	private static double[][] errorOl3 = new double[h2+1][h3];
	private static double[][] errorOl4 = new double[h3+1][ClusterNumber];
	private static double[][] deltaExodus = new double[tes][ClusterNumber];
	private static double[][] deltaProExodus = new double[tes][h3];
	private static double[][] deltaProProExodus = new double[tes][h2];
	private static double[][] deltaProProProExodus = new double[tes][h1];
	
	
	
	public static void main(String[] args) {
		readData("checking", tes);
		//showGraph();
	...
	private static void learn() {
		...
		weight3 = createWeights(h2 + 1, h3);
		weight4 = createWeights(h3 + 1, ClusterNumber);
		int t = 1;
		int y = 0;
		double sfalmaPre = 10;
		double sfalma = 0;
		while(y == 0) {
			...
			newInputs3 = forwardPass(newInputs2, h3, weight3, tes);
			newInputs4 = forwardPass(newInputs3, ClusterNumber, weight4,tes);
			backProp();
			updateWeights();
			for (int i = 0; i < tes; i++) {
				for (int j = 0; j < ClusterNumber; j++) {					
			...
			for (int i = 0; i < tes; i++) {
				for (int j = 0; j < h1; j++) {					
					sfalma += Math.pow(deltaProProProExodus[i][j],2);
				}
			}
			sfalma /= 2;
			t++;
		...
		newInputs3 = forwardPass(newInputs2, h3, weight3, 4000);
		newInputs4 = forwardPass(newInputs3, ClusterNumber, weight4, 4000);
		for (int i = 0; i < 4000; i++) {
			double max=0;
			int teamj=0;
			for (int j = 0  ; j < 4; j++) {
	...
	private static void updateWeights(){
		...
		for (int i = 0; i < h3 + 1; i++) {
			for (int j = 0; j < ClusterNumber; j++) {
				weight4[i][j] = weight4[i][j] - errorOl4[i][j]*n;
			}
		}
	}
	...
	private static void cleaner(){
		...
		errorOl3 = new double[h2+1][h3];
		errorOl4 = new double[h3+1][ClusterNumber];
	...
	private static void backProp(){
		...
		deltaProExodus = calculateLevelError(deltaExodus, h3, ClusterNumber, weight4, newInputs3);
		deltaProProExodus = calculateLevelError(deltaProExodus, h2, h3, weight3, newInputs2);
		deltaProProProExodus = calculateLevelError(deltaProProExodus, h1, h2, weight2, newInputs1);
		...
		for (int k = 0; k < tes; k++) {
			double[][] paragogos = new double[h3+1][ClusterNumber];
			for (int i = 1; i < h3 + 1; i++) {
				for (int j = 0; j < ClusterNumber; j++) {
					paragogos[i][j] = deltaExodus[k][j]*newInputs3[k][i-1];
					errorOl4[i][j] += paragogos[i][j];
				}
			}
		}
		for (int k = 0; k < tes; k++) {
			for (int j = 0; j < ClusterNumber; j++) {
				errorOl4[0][j] += deltaExodus[k][j];
			}
		}
		
		for (int k = 0; k < tes; k++) {
			double[][] paragogos = new double[h2+1][h3];
			for (int i = 1; i < h2 + 1; i++) {
				for (int j = 0; j < h3; j++) {
					paragogos[i][j] = deltaProExodus[k][j]*newInputs2[k][i-1];
		...
		for (int k = 0; k < tes; k++) {
			for (int j = 0; j < h3; j++) {
				errorOl3[0][j] += deltaProExodus[k][j];
			}
		}
		...
		for (int k = 0; k < tes; k++) {
			...
			for (int i = 1; i < h1 + 1; i++) {
				for (int j = 0; j < h2; j++) {
					paragogos[i][j] = deltaProProExodus[k][j]*newInputs1[k][i-1];
		...
		for (int k = 0; k < tes; k++) {
			for (int j = 0; j < h2; j++) {
				errorOl2[0][j] += deltaProProExodus[k][j];
			}
		}
		...
		for (int k = 0; k < tes; k++) {
			...
			for (int i = 1; i < 3; i++) {
				for (int j = 0; j < h1; j++) {
					paragogos[i][j] = deltaProProProExodus[k][j] * newInputs[k][i-1];
		...
		for (int k = 0; k < tes; k++) {
			for (int j = 0; j < h1; j++) {
				errorOl1[0][j] += deltaProProProExodus[k][j];
			}
		}
	}			
		...
		for (int i = 0; i < tes; i++) {
			for (int j = 0; j < ClusterNumber; j++) {
				if (points[i].getClusterId() - 1 == j) {
					if (pickRelu) {
						deltaExodus[i][j] = newInputs4[i][j] - 1;
				
					}
					else {
						deltaExodus[i][j] = newInputs4[i][j] * (1 - newInputs4[i][j]) * (newInputs4[i][j] - 1);
						
					}
				}
				else {
					if (pickRelu) {
						deltaExodus[i][j] = newInputs4[i][j] - 0;
					}
					else {
						deltaExodus[i][j] = newInputs4[i][j] * (1 - newInputs4[i][j]) * (newInputs4[i][j] + 1);
	...
	private static void generateNewData() {
		try {
			dataGenerator();
			FileWriter fileWriter = new FileWriter("DATA/" + "learning" + ".txt");
